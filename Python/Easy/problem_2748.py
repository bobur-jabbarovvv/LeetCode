...
that divides both of them. In other words,
...
Explanation:
    ...
    and the last digit of nums[1] is 5. We can confirm that 2 and 5 are coprime,
    since gcd(2,5) == 1.
...
Explanation:
    ...
    When i = 0 and j = 1: the first digit of nums[0] is 1,
    and the last digit of nums[1] is 1. Indeed, gcd(1,1) == 1.
    When i = 0 and j = 2: the first digit of nums[0] is 1,
    and the last digit of nums[2] is 2. Indeed, gcd(1,2) == 1.
    Thus, we return 2.
"""
...
class Solution(object):
    def gcdOf(self, num1, num2):
        if (num1 < num2):
            return self.gcdOf(num2, num1)
    ...
    def isCoprime(self, num1, num2):
        return self.gcdOf(num1, num2) == 1

    def firstDigitOf(self, num):
        while (num >= 10):
            num //= 10
        return num

    def lastDigitOf(self, num):
        return num % 10
    ...
    def countBeautifulPairs(self, nums):
        ...
        for i in range(len(nums)):
            for j in range(i+1, len(nums)):
                if self.isCoprime(self.firstDigitOf(nums[i]), self.lastDigitOf(nums[j])):
                    counter += 1
        return counter