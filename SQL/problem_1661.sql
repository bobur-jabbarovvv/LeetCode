...
The 'start' timestamp will always be before 
the 'end' timestamp for every (machine_id, process_id) pair.
...
SELECT
    start.machine_id, -- machine id in the 'start' table
    ROUND(AVG(end.timestamp - start.timestamp), 3) AS processing_time
FROM
    Activity start -- 'start' table
JOIN 
    Activity end -- 'end' table
ON
