...
that make orders at some date and specify a preferred delivery date
...
SELECT
    ROUND(AVG(Delivery.order_date = Delivery.customer_pref_delivery_date) * 100, 2) AS immediate_percentage
FROM
    first_orders
JOIN
    Delivery
ON
    first_orders.customer_id = Delivery.customer_id AND
