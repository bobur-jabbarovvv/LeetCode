...
FROM
    Person
-- we need all rows from Person Table and matching rows from Address Table,
-- that's why we use LEFT (OUTER) JOIN
LEFT JOIN
    -- we also can JOIN <condition> ON,
    -- but personId is named same in both tables,
    -- here we have USING for minimalism
